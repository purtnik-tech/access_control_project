...
class Employee(models.Model):
    ...
    login_user: models.CharField = models.CharField(
        max_length=50,
        primary_key=True,  # Логин используется как первичный ключ
        verbose_name="Логин"
    )
    ...
    patronymic: models.CharField = models.CharField(
        max_length=50,
        blank=True,  # Поле может быть пустым (не у всех есть отчество)
    ...
    employee_number: models.CharField = models.CharField(
        max_length=20,
        unique=True,  # Табельный номер должен быть уникальным
...
class Guest(models.Model):
    ...
    organization: models.CharField = models.CharField(
        max_length=100,
        blank=True,  # Не все гости представляют организацию
...
class LicensePlate(models.Model):
    ...
    # Номерной знак (например, "А123ВС177" или "A123BC45")
    plate_number: models.CharField = models.CharField(
        max_length=15,
        unique=True,  # Номер должен быть уникальным в системе
        validators=[
            RegexValidator(
                r'^[А-ЯA-Z0-9]+$',  # Только буквы (русские/латинские) и цифры
...
class Pass(models.Model):
    ...
    # Типы пропусков (выбор из двух вариантов)
    PASS_TYPES: Tuple[Tuple[str, str], ...] = (
        ('permanent', 'Постоянный (сотрудник)'),  # Для сотрудников - бессрочный
        ('temporary', 'Временный (гость)'),  # Для гостей - с ограничениями
    )
    ...
    license_plate: models.ForeignKey = models.ForeignKey(
        ...
        verbose_name="ГНЗ"  # Государственный номерной знак
    )

    # Параметры действия пропуска
    ...
    def clean(self) -> None:
        ...
        Вызывается автоматически при сохранении через формы и админку.

        Raises:
            ValidationError: Если данные не проходят валидацию
        """
        # Проверка: должен быть указан хотя бы один владелец
        if not self.guest and not self.employee:
            raise ValidationError(
                "Должен быть указан либо сотрудник, либо гость."
            )

        # Проверка: у постоянного пропуска не должно быть даты окончания
        if self.pass_type == 'permanent' and self.end_date:
            raise ValidationError(
                "Для постоянного пропуска дата окончания не задаётся."
            )

        # Проверка: у временного пропуска обязательно должна быть дата окончания
    ...
    def __str__(self) -> str:
        ...
        """
        # Получаем владельца (приоритет у сотрудника, если он есть)
